...
public class billDetailsGUI extends JFrame {
	...
	List<String> employeeList = eBLL.geEmployeeList();
	List<String> customerList = uBLL.getUserList();
	...
	public void initComponent() {
		...
		if(employeeInterfaceGUI.employeeCode!=-1) {
			String employeeName = dh.changeCodeToName("employee", "EMPLOYEE_NAME", employeeInterfaceGUI.employeeCode, "EMPLOYEE_CODE");
	...
	private void jTableMouseClicked(MouseEvent e) {
        ...
        if(selectedIndex >= 0) {            
    		...
    		String customerName = dh.changeCodeToName("user", "USER_NAME", customerCode, "USER_CODE");
    		cbEmployee.setSelectedItem(customerName);
    		...
    		if(employeeInterfaceGUI.employeeCode!=-1) {
    			String employeeName1 = dh.changeCodeToName("employee", "EMPLOYEE_NAME", employeeInterfaceGUI.employeeCode, "EMPLOYEE_CODE");
	...
	private void btnAddActionPerformed(ActionEvent e) {
		if(!tfBillDate.getText().trim().equals("") && !tfBillDetailsCode.getText().trim().equals("") && !tfAmount.getText().trim().equals("") && !tfPrice.getText().trim().equals("")) {
			try {
				...
				int customerCode = dh.changeNameToCode("user", "USER_CODE", customerName, "USER_NAME");
				...
				int employeeCode = dh.changeNameToCode("employee", "EMPLOYEE_CODE", employeeName, "EMPLOYEE_NAME");
				...
				BillDetails p = new BillDetails(billDate, billDetailsCode, customerCode, productCode, employeeCode, amount, price, total);
            ...
            if(billDetailsList.size()==0) {
	    		...
	    		while(i < billDetailsList.size()) {
	    			...
	    			model.addRow(new Object [] {
	    					model.getRowCount()+1, p.getBillDate(), p.getBillDetailsCode(), p.getCustomerCode(), p.getProductCode(), p.getEmployeeCode(), p.getAmount(), p.getPrice() , p.getTotal()
	    			});
	    			i++;
	    		}
            }
