...
//		if(!tfAmount.getText().trim().equals("") && Integer.parseInt(tfAmount.getText())>0) {
//			try {
...
//				int total = 0;
//				
//				// thanh toán xóa nó trong giỏ hàng 
//				Cart p = new Cart(cartCode, productCode, productName, amount, price, total);
//				JOptionPane.showMessageDialog(null, cusBLL.cartBay(userCode, p));
//			}catch(NumberFormatException ex) {
//				JOptionPane.showMessageDialog(null, "Số lượng của sản phẩm muốn mua phải là số");
//			}
//		}
//		else {
//			JOptionPane.showMessageDialog(null, "Vui lòng chọn số lượng phù hợp của sản phẩm để thanh toán.");
//		}
	}
            ...
            if(cartList.size()==0) {
	    		...
	    		while(i < cartList.size()) {
	    			...
	    			model.addRow(new Object [] {
	    					model.getRowCount()+1, p.getProductName(),p.getAmount(), p.getPrice() , //p.getTotal
	    			});
	    			i++;
	    		}
            }
	...
	private void displayList() {
		...
		while(i < cartList.size()) {
			...
			model.setValueAt(p.getTotal(), i, 4);
