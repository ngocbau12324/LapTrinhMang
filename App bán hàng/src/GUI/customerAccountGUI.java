...
public class customerAccountGUI extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
					if(tfAccountName.getText().equals("") || tfPassword.getText().equals("") || tfAddress.getText().equals("") || tfNumberPhone.getText().equals("")) {
						...
						c.setCustomerName(tfAccountName.getText().toString());
						c.setAddress(tfAddress.getText().toString());
						c.setNumberPhone(tfNumberPhone.getText().toString());
						
						JOptionPane.showMessageDialog(null, accBLL.updateAccount(p, c, indexUserGUI.userCode, Integer.parseInt(loginGUI.code)));
					}
			}
		});
		btnUpdateInfo.setBounds(224, 407, 279, 50);
	...
	public void display() {
		accountList = accBLL.getAllAccount();
		customerList = userBLL.getAllUser();
		int i = 0;
		...
		while(j < customerList.size()) {
			Customer p = customerList.get(j);
			if(p.getCustomerCode()==indexUserGUI.userCode) {
				tfAccountName.setText(p.getCustomerName());
