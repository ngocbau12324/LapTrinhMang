...
//					employeeCode = dh.changeNameToCode("employee", "EMPLOYEE_CODE", employeeName, "EMPLOYEE_NAME");
//				}
...
//				// check mã sản phẩm
//				if(pDAL.hasProductCode(productCode)) {
//					JOptionPane.showMessageDialog(null, "Mã sản phẩm này đã tồn tại. Vui lòng kiếm tra lại");
//				}
	...
	private void btnSearchActionPerformed(ActionEvent e) {
		String productCode = tfFind.getText();
        if(productCode != null && productCode.length() > 0) {
            ECDetailsList = ECDetailsBLL.searchEnterCouponDetailsByProductCode(productCode);
            ...
            if(ECDetailsList.size()==0) {
	    		...
	    		while(i < ECDetailsList.size()) {
	    			...
	    			model.addRow(new Object [] {
	    					model.getRowCount()+1, p.getCouponDate(), p.getEnterCouponCodeDetails(), p.getProductCode(), p.getSupplierCode(), p.getEmployeeCode(), p.getPrice(), p.getImportPrice(), p.getAmount(), p.getTotal()
	    			});
	    			i++;
	    		}
            }
